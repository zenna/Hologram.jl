...
#draw_points!(heatmap(@. interference_pattern(X,Y,Refx,Refy,all_obj_x[2],all_obj_x[2])'))

# ╔═╡ 015cd0e0-776e-11eb-2c45-592e5cfdee05
#draw_points!(heatmap(@. exposed_film(X,Y,Refx,Refy,all_obj_x[2],all_obj_x[2])'))

# ╔═╡ 0270b1e0-776e-11eb-3778-e12cdfe2d466
#draw_points!(heatmap(@. angle_bounce(X,Y,Refx,Refy,all_obj_x[2],all_obj_x[2])'))

# ╔═╡ 7b0bf770-73ec-11eb-293a-b321fd7bdb3b
alpha_lines = .5; 

# ╔═╡ 7be03480-7767-11eb-18d3-170ce7de4bc5
line_width = .2;
...
# ╔═╡ 13d96da0-7769-11eb-009c-7167678167e5
(all_obj_y)

# ╔═╡ 99073020-71b0-11eb-27b3-4b7ddb9f9836
exposure_time = lambda*10
	...
	for i in xids
		for j in yids
			if exposed_full[i,j]>=thres
				...
				plot!(plt,fxline,1,N; label=false, color=:white, alpha=alpha_lines, linewidth=line_width, xlims=[0,N], ylims=[0,N])
			end
		end
	end
	return plt
	...
	for i in xids
		for j in yids
			if exposed_full[i,j]>=thres
				...
				plot!(plt,fxline,Refx,i; label=false, color=:white, alpha=alpha_lines, linewidth=line_width, xlims=[0,N], ylims=[0,N])
			end
		end
	end
	return plt
